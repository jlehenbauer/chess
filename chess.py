...
class Board:
    ...
    def move(self, notation):
        ...
        # Pawn Move
        if notation[0].lower not in ['k', 'q', 'r', 'b', 'n', 'o']:
            ...
            for file in range(len(self.board)):
                for rank in range(len(self.board[file])):
                    ...
                    if self.col(notation[0]) == rank and piece is not None:
                        if self.TURN == piece.color and piece.name == "Pawn":
                            print(f"moving {piece} to {notation}")
                            self.board[8 - int(notation[1])][rank] = piece
                            self.board[file][rank] = None
                            self.end_turn()
                            return True

        # King move
        elif notation[0].lower() == 'k':
            pass

        # Queen move
        elif notation[0].lower() == 'q':
            pass

        # Rook move
        elif notation[0].lower() == 'r':
            pass

        # Bishop move
        elif notation[0].lower() == 'b':
            pass

        # Knight move
        elif notation[0].lower() == 'n':
            pass

        # TODO: Add Castling Notation Logic (O-O, O-O-O)
